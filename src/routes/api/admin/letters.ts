...
export const Route = createFileRoute("/api/admin/letters")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (!payload?.from || !payload?.body) return json(400, { error: "from, body required" });
        const { token, projectId } = await getToken();
        const res = await fetch(
          `https://firestore.googleapis.com/v1/projects/${encodeURIComponent(projectId)}/databases/(default)/documents/letters`,
          {
            method: "POST",
      ...
      PUT: async ({ request }) => {
        const denied = assertAdmin(request);
        if (denied) return denied;
        const payload = (await request.json().catch(() => null)) as { id?: string; from?: string; body?: string } | null;
        if (!payload?.id || !payload?.from || !payload?.body) return json(400, { error: "id, from, body required" });
        const { token, projectId } = await getToken();
        const res = await fetch(
          `https://firestore.googleapis.com/v1/projects/${encodeURIComponent(projectId)}/databases/(default)/documents/letters/${encodeURIComponent(payload.id)}`,
          {
            method: "PATCH",
            headers: { authorization: `Bearer ${token}`, "content-type": "application/json" },
            body: JSON.stringify({
      ...
      DELETE: async ({ request }) => {
        ...
        const { token, projectId } = await getToken();
        const res = await fetch(
          `https://firestore.googleapis.com/v1/projects/${encodeURIComponent(projectId)}/databases/(default)/documents/letters/${encodeURIComponent(id)}`,
